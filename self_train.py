...
class WrapperEnv(gym.Env):
    ...
    def step(self, action):
        ...
        for _ in range(repeat_actions):
            ...
            ## change reward function if indicated
            if self.changed_reward:
                # green penalty
...
def train_agent(changed_reward = True):
    agent = CarAgent(changed_reward)
    env = WrapperEnv(changed_reward=changed_reward)

    data_dir = 'data/self_changed_reward_train.txt' if changed_reward else 'data/self_unchanged_reward_train.txt'
